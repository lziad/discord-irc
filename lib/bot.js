...
class Bot {
  constructor(options) {
    ...
    this.forwardBots = options.forwardBots;
    this.paeeye = options.paeeye || '(NOFWD)';
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      // Do not send to IRC if the prefix of message matches paeeye.
      if (text.startsWith(this.paeeye)) {
        return;
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    // If the message is from another message forwarding bot, don't add anything
    const withAuthor = this.forwardBots.includes(author) ? withMentions : Bot.substitutePattern(this.formatDiscord, patternMap);
